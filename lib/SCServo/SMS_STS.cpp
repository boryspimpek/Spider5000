...
void SMS_STS::moveServosSyncEx(uint8_t ids[], int numServos, int targetPos[], u16 baseSpeed, u16 accValue) {
    s16 currentPos[numServos];
    u16 speed[numServos];
    s16 position[numServos];
    u8 acc[numServos];
    ...
    // Wywołanie SyncWritePosEx
    SyncWritePosEx(ids, numServos, position, speed, acc);
}